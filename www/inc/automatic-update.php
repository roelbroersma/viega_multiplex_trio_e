<?php
	...
	file_put_contents("/var/log/lighttpd/automatic-update.txt", $data);
	...
	if(file_exists('/tmp/firmware.bin'))
	{
		...
		if($return)
		{
			...
			post_log(101, 'GnuPG signature wrong for firmware.bin');
		...
		else
		{
			...
			exec("/bin/rm /tmp/firmware*");
			post_log(101, 'extracting into directory "/tmp/' . $path . '"');
		}
		...
		if($iot['status'] != 200)
		{
			post_log(101, 'iot module software verification failed');
			$array['status'] = 901;
			$array['duration'] = 0;
			echo json_encode($array);
		...
		{
			post_log(101, 'iot module software verified successfully');
			$array['status'] = 200;
			if($iot['version'] > $iot_version)
				$array['duration'] += 300;
			else
				post_log(101, 'iot module software will not be updated');
		}
		...
		post_log(101, 'Scheduling update in directory ' . $path);
	...
	{
		exec("/bin/rm /tmp/firmware.bin");
		file_put_contents("/tmp/downloaded.txt", "0-0");
		echo json_encode($array);
	}
?>
